...
> VoxType 说明：本页保留的是豆包标准流式 ASR 参考内容。火山方舟 Agent Plan 使用专属 `X-Api-Key`、固定 Resource ID `volc.seedasr.sauc.duration` 和 `wss://openspeech.bytedance.com/api/v3/plan/sauc/bigmodel_async`；以[官方 Agent Plan 文档](https://www.volcengine.com/docs/82379/2516286?lang=zh)为准。两种接入在 VoxType 中共用音频协议和最终包门禁。
