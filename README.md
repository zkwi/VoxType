...
- 自动输入：最终文本写入剪贴板，并用带扫描码和短间隔的 `Ctrl+V` 或 `Shift+Insert` 粘贴到当前焦点输入框；默认自动粘贴后恢复原剪贴板文本。当前只恢复纯文本剪贴板，原剪贴板为图片、文件或富文本时会保留识别文本并提示。
...
保存配置前会做基础字段校验，明显非法的采样率、声道数、录音时长、粘贴延迟、超时时间和悬浮窗尺寸不会写入配置文件。
