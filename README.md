...
- 自动输入：最终文本写入剪贴板，并用带扫描码和短间隔的 `Ctrl+V` 或 `Shift+Insert` 粘贴到当前焦点输入框；首页可临时展开查看最近一次识别文本，关闭窗口或开始下一次录音后清除。默认页只显示“自动粘贴 / 仅复制到剪贴板”，完整粘贴方式和剪贴板恢复延迟在高级设置中调整。
...
- 检查更新：可在选项页高级设置中通过 GitHub Release 检查最新版，并下载后自动启动 Windows 安装包；应用内更新会尽量静默安装并退出当前版本释放文件。
