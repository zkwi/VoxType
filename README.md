...
左侧导航按使用任务拆分为首页、热词、API配置、选项和统计分析：热词页管理热词、场景上下文和提示词，API配置页管理服务密钥、请求参数和健康检查，选项页管理本地启动、输出、录音、悬浮字幕、托盘、更新和诊断。统计页展示最近 24 小时、最近 7 日、平均输入速度和按日使用情况，新识别结果写入后会刷新。
...
- 悬浮字幕：录音时在屏幕居下显示实时识别文本，不抢焦点；选项页默认展示字幕外观，支持预设配色、自定义背景色/字体颜色和透明度预设。
- 自动输入：最终文本写入剪贴板，并用带扫描码和短间隔的 `Ctrl+V` 或 `Shift+Insert` 粘贴到当前焦点输入框；默认自动粘贴后恢复原剪贴板。纯文本恢复最稳定，富文本、表格区域、图片、文件或部分私有格式可能无法完整恢复并提示；可调整恢复延迟和快照大小上限。
...
- 托盘常驻：关闭主窗口默认隐藏到托盘，首次关闭会提示说明；也可在选项页改为直接退出或每次询问。
- 开机启动：可在选项页开启随 Windows 登录自动启动。
- 检查更新：可通过 GitHub Release 检查最新版，并下载后自动启动 Windows 安装包；应用内更新会尽量静默安装并退出当前版本释放文件。
...
- 最近结果提示：识别完成后，首页会保留最近一次输入结果提示；如果目标输入框没有出现文字，可直接按 `Ctrl + V` 手动粘贴。
...
热词页还提供“自动生成热词候选”。该功能默认关闭，开启后只保存 VoxType 自己生成的最终语音输入文本，不记录键盘输入，不读取剪贴板历史。只有用户点击“生成候选”时，才会把本地历史摘要发送到已配置的大模型服务；生成结果只是候选，必须勾选确认后才会合并到 `context.hotwords`。
...
- 最近上下文默认关闭，不保存最近识别片段；需要连续识别增强时可在热词页手动开启。开启后识别片段写入单独的本地 `context/recent_context.jsonl`，不会写回 `config.toml`，也可在热词页清除。
- 自动热词候选默认关闭。开启后本地采集文本写入 `context/hotword_history.jsonl`，可在热词页清空；诊断报告和日志不会输出历史正文、候选词或 prompt。
...
选项页的“诊断与日志”也可以直接打开本地日志或复制诊断报告。日志会记录关键启动阶段、配置保存、ASR/LLM 错误、更新检查和前端异常；密钥形态会自动脱敏。诊断报告默认不包含识别正文、密钥、热词、prompt、最近上下文、自动热词历史正文、候选词或 Windows 用户名路径。
